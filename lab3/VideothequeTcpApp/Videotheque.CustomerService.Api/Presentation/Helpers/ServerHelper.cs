...
using Videotheque.CustomerService.Api.Application.Services.Implementations;
using Videotheque.CustomerService.Api.Infrastructure.Persistence;
using Videotheque.CustomerService.Api.Models.DTO;

namespace Videotheque.CustomerService.Api.Presentation.Helpers
{
    public static class ServerHelper
    {
        ...
        {
            Console.WriteLine($"CustomerService received request at {DateTime.Now:yyyy-MM-dd HH:mm:ss} for path: {client.Client.RemoteEndPoint}");
            ...
            try
            {
                ...
                var customersService = new CustomersService(ctx);

                string response;
                Console.WriteLine($"IdPart: {method} {path}");
                if (method == "GET" && path == "/customers")
                {
                    var all = customersService.GetAll();
                    Console.WriteLine($"Retrieved {all?.Count() ?? 0} customers");
                    response = CreateResponse("200 OK", all);
                }
                else if (method == "GET" && path.StartsWith("customers/"))
                {
                    var idPart = path.Substring("customers/".Length);
                    if (!Guid.TryParse(idPart, out var id))
                        response = CreateResponse("400 Bad Request");
                    else
                    {
                        var customer = customersService.GetById(id);
                        response = customer != null ? CreateResponse("200 OK", customer) : CreateResponse("404 Not Found");
                    }
                }
                else if (method == "POST" && path == "/customers")
                {
                    ...
                    else
                    {
                        ...
                        try
                        {
                            var dto = JsonSerializer.Deserialize<CustomerDTO>(body, _jsonOptions);
                            if (dto == null)
                            {
                            ...
                            {
                                var created = customersService.Create(dto);
                                Console.WriteLine($"Customer created: {created?.Name}");
                ...
                else if (method == "PUT" && path.StartsWith("/customers/"))
                {
                    ...
                    else
                    {
                        ...
                        {
                            dto.Id = id;
                            var ok = customersService.Update(dto);
                ...
                else if (method == "DELETE" && path.StartsWith("/customers/"))
                {
                    ...
                    {
                        var success = customersService.Delete(id);
                        if (success)
                        {
